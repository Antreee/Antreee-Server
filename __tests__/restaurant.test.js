...
describe("- Success Get Restaurants -", () => {
	...
	describe("- Success update Restaurant Availability -", () => {
		...
		it("patch update Restaurant Availability - success patch restaurant", async () => {
			const loginResponse = await request(app).post("/admin/login").send(user);
			const access_token = loginResponse.body.access_token;
			const res = await request(app)
...
describe("get restaurant --- failed", () => {
	...
	describe("- Success Get Restaurant booked By Restaurant Id -", () => {
		it("get restaurant booked by restaurant id - success get restaurant", async () => {
			const payload = {
				email: "domino@admin.com",
				password: "domino",
			};
			...
			const access_token = loginResponse.body.access_token;
			const res = await request(app)
				.get("/restaurants/625a63636ab2f903486ecc74/booked")
				.set("access_token", access_token);
			...
			expect(res.body).toHaveProperty("message", expect.any(String));
			expect(res.body).toHaveProperty("message", res.body.message);
			expect(res.body).toHaveProperty("booked", expect.any(Array));
		});
	});

	describe("- fail Get Restaurant booked By Restaurant Id -", () => {
		it("get restaurant booked by restaurant id - failed get restaurant", async () => {
			const payload = {
				email: "domino@admin.com",
				password: "domino",
			};
			...
			const access_token = loginResponse.body.access_token;
			const res = await request(app)
				.get("/restaurants/625a63636ab2f903486ecc80/booked")
				.set("access_token", access_token);
...
describe("Get restaurant by id --- Failed", () => {
	...
	it("faill get restaurant by id with wrong id", async () => {
		const res = await request(app).get("/restaurants/123sdfbf34er45");
		const { status, body } = res;
		expect(status).toBe(400);
		expect(body).toBeInstanceOf(Object);
		expect(body).toHaveProperty("message", "Invalid restaurant id");
